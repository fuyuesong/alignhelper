...
def asr_audio_baidu(fpaths, outpath):
    ...
    outs = []
    for fpath in tqdm(fpaths):
        out = request_one_baidu(fpath=fpath)
...
def get_fpaths_asr(indir, inpath):
    ...
    outs = []
    for fpath in tqdm(sorted(Path(indir).glob("*.wav"))):
...
if __name__ == "__main__":
    ...
    # inpath = r'data\王凯\【王凯】我在等待中变得更好_de-noise.wav'
    # outpath = f'../data/{name}/{name_pinyin}.mp3'
    # convert_audio(inpath, outpath)
    #
    # inpath = f'../data/{name}/{name_pinyin}.mp3'
    # outdir = f'../data/{name}/{name_pinyin}'
    # Path(outdir).mkdir(exist_ok=True)
    # split_audio(inpath, outdir)

    # input('继续执行请Enter...')

    indir = f'../data/{name}/{name_pinyin}'
    inpath = f'../data/{name}/{name_pinyin}_info.csv'
    fpaths = get_fpaths_asr(indir, inpath)

    # outpath = f'../data/{name}/{name_pinyin}_json.txt'
    # asr_audio_xunfei(fpaths, outpath)
    #
    # inpath = f'../data/{name}/{name_pinyin}_json.txt'
    # outpath = f'../data/{name}/{name_pinyin}_info.csv'
    ...
    inpath = f'../data/{name}/{name_pinyin}_info.csv'
    outpath = f'../data/{name}/{name_pinyin}_info_pinyin.csv'
    pinyin_the_text(inpath, outpath)

    inpath = f'../data/{name}/{name_pinyin}_info_pinyin_check.csv'
    outpath = f'../data/{name}/{name_pinyin}_ssml.txt'
